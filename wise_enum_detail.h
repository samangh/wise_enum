...
// Needed for exapnsion of variadic macro arguments in MSVC
...
#define WISE_ENUM_IMPL_NARG_(...) EXPAND(WISE_ENUM_IMPL_ARG_N(__VA_ARGS__))
...
#define WISE_ENUM_IMPL_CHECK(...) EXPAND(WISE_ENUM_IMPL_CHECK_N(__VA_ARGS__, 0, ))
...
#define WISE_ENUM_IMPL_3(type, name, storage, friendly, num_enums, loop, ...)  \
  type name storage{                                                           \
      EXPAND(loop(WISE_ENUM_IMPL_ENUM_INIT, _, WISE_ENUM_IMPL_COMMA,           \
              __VA_ARGS__))};                                                  \
...
#define WISE_ENUM_IMPL_ADAPT_3(name, friendly, num_enums, loop, ...)           \
  ...
  wise_enum_detail_array(::wise_enum::detail::Tag<name>) {                     \
    return {{EXPAND(loop(WISE_ENUM_IMPL_DESC_PAIR, name, WISE_ENUM_IMPL_COMMA,  \
                  __VA_ARGS__))}};                                              \
  }                                                                            \
                                                                               \
  template <class T>                                                           \
  ...
  wise_enum_detail_to_string(T e, ::wise_enum::detail::Tag<name>) {            \
    switch (e) {                                                               \
      EXPAND(loop(WISE_ENUM_IMPL_SWITCH_CASE, name, WISE_ENUM_IMPL_NOTHING,     \
           __VA_ARGS__))                                                        \
    }                                                                          \
    return {};                                                                 \
  }
